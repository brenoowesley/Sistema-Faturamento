...
import { ChevronDown, ChevronUp, History, CheckCircle2, XCircle, AlertTriangle } from "lucide-react";
        ...
        >
            ...
            >
                <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                    ...
                    <span style={{ fontWeight: 600, color: "var(--fg)", fontSize: 15 }}>{lote.nome_lote}</span>
                    <span className="badge badge-info" style={{ fontSize: 11 }}>{lote.status}</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                    <span style={{ fontSize: 12, color: "var(--fg-dim)" }}>{fmtDatetime(lote.created_at)}</span>
                                            ...
                                            <tbody>
                                                {approved.map((i) => (
                                                    <tr key={i.id}>
                                                        ...
                                                        <td style={{ color: "var(--success)", fontWeight: 600 }}>R$ {Number(i.valor).toFixed(2)}</td>
                                                        <td className="table-mono">{i.data_solicitacao ? fmtDatetime(i.data_solicitacao) : "—"}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                                            ...
                                            <tbody>
                                                {blocked.map((i) => (
                                                    <tr key={i.id}>
                                                        <td className="table-mono">{i.cpf_favorecido}</td>
                                                        <td className="table-mono" style={{ fontSize: 12 }}>{i.chave_pix || "—"}</td>
                                                        <td style={{ color: "var(--danger)", fontWeight: 600 }}>R$ {Number(i.valor).toFixed(2)}</td>
    ...
    return (
        <div>
            ...
            {loading ? (
                ...
                lotes.map((lote) => <LoteRow key={lote.id} lote={lote} />)
            )}
        </div>
    );
}
