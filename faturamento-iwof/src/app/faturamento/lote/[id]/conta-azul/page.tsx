...
export default function ContaAzulStagingPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const mappedRows: ContaAzulRow[] = (records || []).map(r => {
                ...
                return {
                    ...
                    categoria: `FATURAMENTO ${currentLote.ciclo}`,
    ...
    const handleApplyBulk = () => {
        if (!confirm(`Deseja aplicar estas alterações a todas as ${filteredData.length} linhas visíveis?`)) return;

        const updatedIds = new Set(filteredData.map(f => f.id));
    ...
    const handleExport = () => {
        const exportData = filteredData.map(r => ({
            "Data de Competência": r.dataCompetencia,
                ...
                <section className="bg-[var(--bg-card)] rounded-3xl border border-[var(--border)] p-6 shadow-2xl">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                        ...
                        <div className="flex items-end">
                            ...
                            >
                                <CheckCircle2 size={18} /> Aplicar aos Filtrados
                            </button>
                        </div>
                    </div>
                </section>
                        ...
                        <table className="w-full text-left border-collapse min-w-[1200px]">
                            <thead>
                                <tr className="bg-[var(--bg-main)]/50 text-[10px] uppercase font-bold text-[var(--fg-dim)] tracking-widest border-b border-[var(--border)]">
                                    <th className="p-4 w-32">Competência</th>
                                    ...
                                    <th className="p-4 w-24 text-center">CC</th>
                                    <th className="p-4 min-w-[200px]">Observações</th>
                                </tr>
                            </thead>
                            <tbody>
                                {filteredData.map(row => (
                                    <tr key={row.id} className="border-b border-[var(--border)]/50 hover:bg-white/[0.02] transition-colors group">
                                        <td className="p-2">
                                            <input
                                                type="date"
